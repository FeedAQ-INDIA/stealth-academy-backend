...
async function createCourseFromUrls(req, res, next) {
    ...
    try {
        ...
        logger.info(`🎥 Creating course from YouTube URLs for user: ${req.user.userId}`);
        ...
        const result = await YoutubeService.createStructuredCourse(
            contentUrlsList,
            courseTitle,
            courseDescription,
            req.user.userId
        ...
        if (err.message?.includes('No valid videos found')) {
            return res.status(400).send({
                status: 400,
                message: "No valid YouTube videos found in the provided URLs. Please check your links.",
            });
        }

        if (err.message?.includes('Database error') || err.message?.includes('SequelizeError')) {
            return res.status(500).send({
                status: 500,
                message: "Database error occurred while creating the course. Please try again.",
            });
        }
        ...
        res.status(500).send({
            status: 500,
            message: err.message || "An error occurred while creating the course from YouTube URLs.",
        });
